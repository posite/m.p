...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = "투표 결과"
        var imgBest = findViewById<ImageView>(R.id.imgBest)
        var tvBest = findViewById<TextView>(R.id.tvBest)
        ...
        for (i in voteResult.indices) {
            ...
            if(voteResult[i]>count){
                count=voteResult[i]
                num=i
            }
        }
        tvBest.setText(imageName[num])
        imgBest.setImageResource(imageFileId[num])
